...
const noDictExp = (e: Program | Exp): boolean =>
  isAtomicExp(e)
    ...
    ? true
    : isIfExp(e)
    ? noDictExp(e.test) && noDictExp(e.then) && noDictExp(e.alt)
    : isProcExp(e)
    ? all((b) => noDictExp(b), e.body)
    : isAppExp(e)
    ? noDictExp(e.rator) && all((rand) => noDictExp(rand), e.rands)
    : isDefineExp(e)
    ? noDictExp(e.val)
    : isLetExp(e)
    ? all(
        (val: CExp) => noDictExp(val),
        map((b: Binding) => b.val, e.bindings)
      ) && all((b) => noDictExp(b), e.body)
    : isDictExp(e)
    ? true
    : isProgram(e)
    ? all((e) => noDictExp(e), e.exps)
    : true;

const evalP = (x: string): Result<Value> =>
  bind(parseL32(x), (prog) => evalL32program(L32toL3(prog)));

const noDict = (x: string): Result<boolean> =>
  bind(parseL32(x), (prog) => makeOk(noDictExp(L32toL3(prog))));
  ...
  it("Q23 tests 2", () => {
    ...
    expect(
      evalP(`(L32
                      ...
                      (define y "b")
                      ((dict (a x) (b y)) 'b))`)
    ).to.deep.equal(makeOk(makeSymbolSExp("y")));
  });
