...
;;         |  (dict (<var> <cexp>)*)        / DictExp(entries: {key: string, val: CExp[]}[])
...
export type CompoundExp = AppExp | IfExp | ProcExp | LetExp | LitExp | DictExp; // Added dict here
...
// Dict/////////////////////////////////////////////
export type DictEntry = { key: string; val: CExp };
...
export const makeBoolExp = (b: boolean): BoolExp => ({
  tag: "BoolExp",
  val: b,
});
...
export const isCompoundExp = (x: any): x is CompoundExp =>
  ...
  isLitExp(x) ||
  isLetExp(x) ||
  isDictExp(x); // Added dict pred here
...
export const parseL32Program = (sexp: Sexp): Result<Program> =>
  sexp === "" || isEmpty(sexp)
    ...
    ? parseL32GoodProgram(first(sexp), rest(sexp))
    : makeFailure(`Unexpected type ${format(sexp)}`);

const parseL32GoodProgram = (keyword: Sexp, body: Sexp[]): Result<Program> =>
  keyword === "L32" && !isEmpty(body)
    ? mapv(mapResult(parseL32Exp, body), (exps: Exp[]) => makeProgram(exps))
    : makeFailure(
        `Program must be of the form (L32 <exp>+): ${format([
          keyword,
          ...body,
        ])}`
      );
...
export const parseL32Exp = (sexp: Sexp): Result<Exp> =>
  isCompoundSexp(sexp)
    ? isNonEmptyList<Sexp>(sexp)
      ? parseL32CompoundExp(first(sexp), rest(sexp))
...
export const parseL32CompoundExp = (op: Sexp, params: Sexp[]): Result<Exp> =>
  op === "define" ? parseDefine(params) : parseL32CompoundCExp(op, params);
...
const parseGoodDefine = (variable: Sexp, val: Sexp): Result<DefineExp> =>
  !isIdentifier(variable)
    ? makeFailure(
        `First arg of define must be an identifier: ${format(variable)}`
      )
...
export const parseL32CExp = (sexp: Sexp): Result<CExp> =>
  isCompoundSexp(sexp)
    ? isNonEmptyList<Sexp>(sexp)
      ? parseL32CompoundCExp(first(sexp), rest(sexp))
...
const isSpecialForm = (x: string): boolean =>
  ["if", "lambda", "let", "quote", "dict"].includes(x); // Added dict here
...
const parseIfExp = (params: Sexp[]): Result<IfExp> =>
  params.length !== 3
    ? makeFailure(
        `Expression not of the form (if <cexp> <cexp> <cexp>): ${format(
          params
        )}`
      )
...
const parseProcExp = (vars: Sexp, body: Sexp[]): Result<ProcExp> =>
  isArray(vars) && allT(isString, vars)
    ? mapv(mapResult(parseL32CExp, body), (cexps: CExp[]) =>
...
const parseLetExp = (bindings: Sexp, body: Sexp[]): Result<LetExp> => {
  ...
  const bindingsResult = mapv(valsResult, (vals: CExp[]) =>
...
// Added dict here
const parseDictEntry = (entry: Sexp): Result<DictEntry> =>
  entry.length === 2 && isIdentifier(entry[0])
    ? mapv(parseL32CExp(entry[1]), (val: CExp) => ({
        key: entry[0] as string,
        val,
      }))
    : makeFailure(`Invalid dict entry: ${entry}`);
// Added dict here
const parseDictExp = (entries: Sexp[]): Result<DictExp> =>
  allT(isNonEmptyList, entries)
    ? mapv(mapResult(parseDictEntry, entries), makeDictExp)
    : makeFailure(`Each dict entry must be a list of two elements`);

export const isDottedPair = (sexps: Sexp[]): boolean =>
  sexps.length === 3 && sexps[1] === ".";
...
const unparseLExps = (les: Exp[]): string => map(unparseL32, les).join(" ");
...
const unparseLetExp = (le: LetExp): string =>
  `(let (${map(
    (b: Binding) => `(${b.var.var} ${unparseL32(b.val)})`,
    le.bindings
  ).join(" ")}) ${unparseLExps(le.body)})`;

// Added dict unparser here
const unparseDictExp = (de: DictExp): string =>
  `(dict ${de.entries
    .map((entry) => `(${entry.key} ${unparseL32(entry.val)})`)
    .join(" ")})`;
...
export const unparseL32 = (exp: Program | Exp): string =>
  isBoolExp(exp)
    ...
    ? unparseProcExp(exp)
    : isIfExp(exp)
    ? `(if ${unparseL32(exp.test)} ${unparseL32(exp.then)} ${unparseL32(
        exp.alt
      )})`
    : isAppExp(exp)
    ? `(${unparseL32(exp.rator)} ${unparseLExps(exp.rands)})`
    : isPrimOp(exp)
    ? exp.op
    : isLetExp(exp)
    ? unparseLetExp(exp)
    : isDictExp(exp) // Added dict here
    ? unparseDictExp(exp)
    : isDefineExp(exp)
    ? `(define ${exp.var.var} ${unparseL32(exp.val)})`
    : isProgram(exp)
    ? `(L32 ${unparseLExps(exp.exps)})`
    : exp;
