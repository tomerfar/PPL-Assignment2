...
export const parseL31Program = (sexp: Sexp): Result<Program> =>
  sexp === "" || isEmpty(sexp)
    ...
    ? parseL31GoodProgram(first(sexp), rest(sexp))
    : makeFailure(`Unexpected type ${format(sexp)}`);

const parseL31GoodProgram = (keyword: Sexp, body: Sexp[]): Result<Program> =>
  keyword === "L31" && !isEmpty(body)
    ? mapv(mapResult(parseL31Exp, body), (exps: Exp[]) => makeProgram(exps))
    : makeFailure(
        `Program must be of the form (L31 <exp>+): ${format([
          keyword,
          ...body,
        ])}`
      );
...
export const parseL31Exp = (sexp: Sexp): Result<Exp> =>
  isCompoundSexp(sexp)
    ? isNonEmptyList<Sexp>(sexp)
      ? parseL31CompoundExp(first(sexp), rest(sexp))
...
export const parseL31CompoundExp = (op: Sexp, params: Sexp[]): Result<Exp> =>
  op === "define" ? parseDefine(params) : parseL31CompoundCExp(op, params);
...
export const parseL31CExp = (sexp: Sexp): Result<CExp> =>
  isCompoundSexp(sexp)
    ? isNonEmptyList<Sexp>(sexp)
      ? parseL31CompoundCExp(first(sexp), rest(sexp))
...
const parseProcExp = (vars: Sexp, body: Sexp[]): Result<ProcExp> =>
  isArray(vars) && allT(isString, vars)
    ? mapv(mapResult(parseL31CExp, body), (cexps: CExp[]) =>
